...
# 配置参数
SOURCE_URL = "https://github.com/q1017673817/iptvz/blob/main/zubo_all.txt"
RAW_SOURCE_URL = "https://raw.githubusercontent.com/q1017673817/iptvz/main/zubo_all.txt"  # 原始文件URL
OUTPUT_FILE = "sorted_live_sources.txt"

# 频道名称映射规则（简化版，完整映射见文档）
CHANNEL_MAPPING = {
...
CATEGORY_MAPPING = {
    ...
    "卫视,#genre#": [
        "山东卫视","湖南卫视","浙江卫视","江苏卫视","东方卫视","北京卫视","深圳卫视","广东卫视","大湾区卫视","广西卫视","东南卫视","海南卫视","河北卫视","河南卫视","湖北卫视","江西卫视","四川卫视","重庆卫视","贵州卫视","云南卫视","天津卫视","安徽卫视","辽宁卫视","黑龙江卫视","吉林卫视","内蒙古卫视","宁夏卫视","山西卫视","陕西卫视","甘肃卫视","青海卫视","新疆卫视","西藏卫视","三沙卫视","兵团卫视","延边卫视","安多卫视","康巴卫视","农林卫视","海峡卫视","山东教育卫视"
        # 完整卫视列表
    ],
"4K,#genre#": [
...
def process_raw_data(raw_lines):
    ...
    for line in raw_lines:
        ...
        # 识别地区运营商行
        if part2 == "#genre#":
            current_operator = part1
            continue
        
        # 处理频道行
        channel_name = normalize_channel_name(part1)
        channel_url = part2
        # 生成标准格式行
        standard_line = f'"{channel_name}","{channel_url}"$"{current_operator}"'
        result.append(standard_line)
...
def classify_channels(processed_lines):
    ...
    # 合并分类结果（包含未分类）
    final_result = []
    for category, lines in classified.items():
        if lines:
            final_result.append(f'"{category}"')
            final_result.extend(lines)
    if unclassified:
        final_result.append('"未分类,#genre#"')
        final_result.extend(unclassified)
...
def main():
    """主执行函数"""
    # 1. 获取原始数据
    raw_lines = fetch_raw_content(RAW_SOURCE_URL)
    if not raw_lines:
        print("无有效数据，程序终止")
        return
    
    # 2. 数据格式转换
    processed_data = process_raw_data(raw_lines)
    if not processed_data:
        print("数据处理失败")
        return
    
    # 3. 频道分类
    classified_data = classify_channels(processed_data)
    
    # 4. 写入输出文件
    write_to_file(classified_data, OUTPUT_FILE)
    print("处理完成！")
